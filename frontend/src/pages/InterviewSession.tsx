...
import { getLanguageConfig, getLanguageLabel, getSarvamLanguageCode } from '../lib/interview';
...
export default function InterviewSession() {
  ...
  const [questionsSource, setQuestionsSource] = useState<'sarvam' | 'unavailable'>('unavailable');
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      ...
      try {
        ...
        if (result.source !== 'sarvam' || result.questions.length !== 8) {
          throw new Error('AI did not return a full question set.');
        }

        setQuestions(result.questions);
        setQuestionsSource('sarvam');
      } catch (error) {
        console.error(error);
        if (isMounted) {
          setQuestions([]);
          setQuestionsSource('unavailable');
  ...
  const startInterview = async () => {
    ...
    if (questions.length !== 8 || isGeneratingQuestions || Boolean(questionError)) {
      return;
    }
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
            ...
            <div className="flex flex-wrap items-center gap-3 px-5 py-4">
              {!isInterviewActive ? (
                <button
                  ...
                  disabled={!sessionReady || Boolean(cameraError) || isGeneratingQuestions || questions.length !== 8 || !sarvamLanguageCode || Boolean(questionError)}
          ...
          <section className="space-y-6">
            <div className="rounded-[28px] border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
              ...
              {questionError && (
                <p className="mt-2 text-xs text-rose-300">{questionError}</p>
              )}
              <p className="mt-2 text-xs text-slate-500">
                {isGeneratingQuestions
                  ...
                  : questionsSource === 'sarvam'
                    ...
                    : 'Interview cannot start until AI questions are generated.'}
              </p>
            </div>
